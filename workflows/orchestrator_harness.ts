...
async function orchestrator(): Promise<Result[]>{
  ...
  for (let test of TEST_DATA) {
    ...
    //get the results
    const orch_result: OrchFlowOutput
    = await runOrchestratorWorkflow(input);
    ...
    if (orch_result.classifyResponse === 'time') {
      ...
      const temporal_result : TemporalOutput | undefined = orch_result.temporalResponse;
      ...
      continue;
    } else if (orch_result.classifyResponse === 'research') {
      if (orch_result.ragResults) {
        const companies = orch_result.ragResults.sources.map(s => s.companyName);
