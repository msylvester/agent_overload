...
import { runResearchWorkflow, WorkflowOutputSchema } from './agentic_research_workflow';
...
async function runSingleTest(queryData: TestQuery): Promise<TestResult> {
  ...
  try {
    ...
    // Handle research intent (NEW: Simple string answer from agentic RAG)
    console.log(`\nAgentic RAG Answer:`);

    if (!result.ragResults) {
      console.log(`  Warning: No RAG results received`);
      return {
        query,
        success: false,
        found_expected: false,
    ...
    // ragResults is now a simple string (natural language answer)
    const ragAnswer = result.ragResults;
    console.log(`  ${ragAnswer.substring(0, 500)}${ragAnswer.length > 500 ? '...' : ''}`);

    // Check if expected result is in the answer (for research queries)
    const foundExpected = ragAnswer.toLowerCase().includes(expected.toLowerCase());

    if (foundExpected) {
      console.log(`\n✓ Found expected company: ${expected}`);
    } else {
      console.log(`\n✗ Expected company not found: ${expected}`);
    }
    ...
    return {
      ...
      companies_count: 0, // No longer tracking individual companies (answer is a string)
      result
    };
