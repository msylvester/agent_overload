...
async function generateMermaidDiagram(workflowPath: string) {
  try {
    ...
    let graph = null;
    const possibleExports = ['app', 'workflow', 'graph', 'agenticApp', 'default'];
    ...
    if (!graph) {
      ...
      console.error('   Expected exports: app, workflow, graph, agenticApp, or default');
    ...
    if (typeof graph.getGraph !== 'function') {
      console.error('❌ The exported object does not have a getGraph() method.');
      console.error('   Make sure to export the compiled graph (workflow.compile())');
      process.exit(1);
    }
    ...
    try {
      ...
      console.warn('⚠️  Could not convert to PNG. The .mmd file has been saved.');
    ...
    console.log(`   - ${path.join(workflowDir, `${workflowName}.svg`)} (SVG image)`);
...
if (args.length === 0) {
  ...
  console.error('\nExample:');
  console.error('  npx tsx mermaid_run.ts temporal_router_integration_workflow.ts');
  process.exit(1);
}
