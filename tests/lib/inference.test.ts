...
describe("inference", () => {
  ...
  it("should handle errors from the model", async () => {
    ...
    await expect(inference(testPrompt)).rejects.toThrow(
  ...
  it("should wrap errors with descriptive message", async () => {
    ...
    await expect(inference(testPrompt)).rejects.toThrow(
      "Model inference failed: Network timeout"
    );
  });
  ...
  it("should call generateText with onnx-community/Llama-3.2-1B-Instruct model when specified", async () => {
    ...
    const expectedResponse =
      "Who is Barry Bonds? Barry Bonds is a former professional baseball player who played for the Pittsburgh Pirates and San Francisco Giants.";
